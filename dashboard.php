...
<?php if ($role === 'Worker'): ?><img class="dash-avatar" src="profile.jpg" alt="<?php echo e($user['full_name']); ?>"><?php endif; ?>
<div>
<h1><?php echo e($role === 'Employer' ? 'User Dashboard' : ($role === 'Worker' ? $user['full_name'] : 'Admin Dashboard')); ?></h1>
<p class="muted"><?php echo e($user['full_name']); ?> | Live dashboard refreshes every 30 seconds.</p>
</div>
</div>
<a class="nav-link" href="Jobs page/jobs.php">Browse Jobs</a>
</div>
...
<button formaction="accept_booking.php" type="submit">Accept Request</button>
<button class="danger" formaction="reject_booking.php" type="submit">Reject Request</button>
</form>
<?php endif; ?>
<?php if (($role === 'Worker' || $role === 'Admin') && $booking['status'] === 'Accepted'): ?>
<form class="actions" action="update_status.php" method="POST">
<input type="hidden" name="booking_id" value="<?php echo e($booking['booking_id']); ?>">
...
<?php
$reviewStmt = mysqli_prepare($conn, 'SELECT review_id FROM reviews WHERE booking_id = ? AND employer_id = ? LIMIT 1');
mysqli_stmt_bind_param($reviewStmt, 'ii', $booking['booking_id'], $userId);
mysqli_stmt_execute($reviewExists = $reviewStmt);
...
<?php else: ?>
<form class="actions" action="submit_review.php" method="POST">
<input type="hidden" name="booking_id" value="<?php echo e($booking['booking_id']); ?>">
<input type="hidden" name="worker_id" value="<?php echo e($booking['worker_id']); ?>">
...
<textarea name="review_comment" rows="2" placeholder="Share your review" required></textarea>
