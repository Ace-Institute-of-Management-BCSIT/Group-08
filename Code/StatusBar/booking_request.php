...
if ($jobId <= 0 || $workerId <= 0 || $categoryId <= 0 || !valid_date($requestedDate) || !valid_time($requestedTime) || $offeredSalary < 0 || strlen($notes) > 2000) {
    ...
    mysqli_stmt_execute($validRequest);
    $isValidRequest = mysqli_fetch_assoc(mysqli_stmt_get_result($validRequest));
    mysqli_stmt_close($validRequest);
    if (!$isValidRequest) {
        $status = 'The selected worker is not available for this service.';
    } else {
