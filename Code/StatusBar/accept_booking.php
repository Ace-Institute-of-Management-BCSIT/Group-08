...
try {
    $stmt = mysqli_prepare($conn, 'SELECT booking_id FROM booked_dates WHERE worker_id = ? AND booking_date = ? LIMIT 1 FOR UPDATE');
    mysqli_stmt_bind_param($stmt, 'is', $booking['worker_id'], $bookingDate);
    mysqli_stmt_execute($stmt);
    $alreadyBooked = mysqli_fetch_assoc(mysqli_stmt_get_result($stmt));
    mysqli_stmt_close($stmt);
    if ($alreadyBooked) throw new RuntimeException('already-booked');
    ...
    if (!mysqli_stmt_execute($stmt)) throw new RuntimeException('already-booked');
    mysqli_stmt_close($stmt);
...
mysqli_stmt_execute($exchange);
mysqli_stmt_close($exchange);
