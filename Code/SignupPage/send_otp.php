...
try {
    ...
    if (sendMail($email, $subject, $body)) {
        ...
        echo json_encode(['success' => false, 'error' => 'Failed to send OTP. The site administrator must configure SMTP credentials in config.php.']);
    }
} catch (Throwable $e) {
    error_log('OTP request failed: ' . $e->getMessage());
    http_response_code(500);
    echo json_encode(['success' => false, 'error' => APP_DEBUG ? ('Server error: ' . $e->getMessage()) : 'Server error. Please try again later.']);
}
