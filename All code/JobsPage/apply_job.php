<?php
/**
 * Processes job application submissions and related uploaded documents.
 */
...
if (!empty($_FILES['resume']['name'])) {
    ...
    if ($file['error'] === UPLOAD_ERR_OK && allowed_upload_extension($file, $allowedDocs)) {
        ...
        $relativePath = save_uploaded_file($file, 'JobsPage/uploads/resumes', (int) $user['user_id'], $allowedDocs);
        if ($relativePath) {
        ...
        header('Location: ../AboutUsPage/apply_resume.php?job_id=' . $jobId . '&error=file');
        exit();
    }
} else {
...
if (!empty($_FILES['police_report']['name'])) {
    ...
    $policeReportPath = save_uploaded_file($_FILES['police_report'], 'JobsPage/uploads/police_reports', (int) $user['user_id'], $allowedDocs);
    if (!$policeReportPath) {
        header('Location: ../AboutUsPage/apply_resume.php?job_id=' . $jobId . '&error=upload');
        exit();
    }
}

if (empty($_FILES['citizenship_card']['name'])) {
    header('Location: ../AboutUsPage/apply_resume.php?job_id=' . $jobId . '&error=citizenship');
    exit();
}
...
$citizenshipPath = save_uploaded_file($_FILES['citizenship_card'], 'JobsPage/uploads/citizenship', (int) $user['user_id'], $allowedDocs);
if (!$citizenshipPath) {
    header('Location: ../AboutUsPage/apply_resume.php?job_id=' . $jobId . '&error=upload');
    exit();
}
